...
/**
 * REST resource for analyzing patent documents using Google Gemini AI.
 * Provides endpoints for uploading and analyzing PDF patent agreements.
 */
@Path("/analyze-patent")
@JBossLog
public class PatentAnalyzerResource {
    ...
    /**
     * Uploads and analyzes a PDF patent document.
     * 
     * @param fileUploadRequest The multipart form request containing the PDF file
     * @return JSON string containing the analyzed patent information
     */
    @PUT
    @Consumes(MediaType.MULTIPART_FORM_DATA)
    @Produces(MediaType.TEXT_PLAIN)
    @Path("/upload")
    @Operation(summary = "Upload and analyze a patent document", description = "Uploads a PDF patent document and analyzes it using Google Gemini AI to extract key information")
    @RequestBody(content = @Content(mediaType = MediaType.MULTIPART_FORM_DATA, schema = @Schema(implementation = FileUpload.class)))
   // @APIResponse(responseCode = "200", description = "Successfully analyzed patent document", content = @Content(mediaType = MediaType.TEXT_PLAIN, schema = @Schema(implementation = PatentReport.class)))
        ...
        try {
            ...
            ChatRequest chatRequest = ChatRequest.builder()
                    ...
                    .parameters(ChatRequestParameters.builder()
                            .responseFormat(responseFormatFrom(PatentReport.class))
                            .build())
                    .build();
